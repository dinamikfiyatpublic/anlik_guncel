...
def main():
    ...
    workflow_mapping = {
        "concurrent_run_ana_api_matrix.mjs": "concurrent_run_ana_api_matrix.yml"
    }
    ...
    while True:
        ...
        if not relevant_running_workflows:
            ...
            if scrape_kalan > 0:
                if first_run:
                    # For the first run, only trigger 'concurrent_run_ana_api_matrix.yml'
                    logging.info("First run: triggering 'concurrent_run_ana_api_matrix.yml' workflow.")
                    trigger_workflow("concurrent_run_ana_api_matrix.yml")
                    wait_for_workflow_completion("concurrent_run_ana_api_matrix.mjs")  # Wait for this workflow to finish
                    first_run = False  # First run is done now
                else:
                    # For subsequent runs, trigger the concurrent run again
                    logging.info(f"scrape_kalan is {scrape_kalan}. Triggering 'concurrent_run_ana_api_matrix.yml' workflow.")
                    trigger_workflow("concurrent_run_ana_api_matrix.yml")
                    wait_for_workflow_completion("concurrent_run_ana_api_matrix.mjs")
            else:
