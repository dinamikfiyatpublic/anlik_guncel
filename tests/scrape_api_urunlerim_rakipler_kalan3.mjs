    ...
    try {
        ...
        while (hasMoreData) {
            ...
            const res = await client.query(`
                ...
                SELECT
                    max_sirala,
                    (max_sirala / 10.0) AS parca_boyutu
                FROM max_sirala
                ),
                sirala_sinirlar AS (
                SELECT
                    CEIL(parca_araliklari.parca_boyutu * 2) + 1 AS baslangic,
                    CEIL(parca_araliklari.parca_boyutu * 3) AS bitis
...
async function processLinkInChildProcess(link, kategoriAna, kategoriAlt, kategoriAlt2, marka, urun_kodu, timestamp, sira, p_adi) {
    return new Promise((resolve, reject) => {
        ...
        const child = fork('./scrape_api_urunlerim_rakipler.mjs', [
